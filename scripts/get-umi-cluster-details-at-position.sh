...
bamFile=${2/.bam/}-$1.bam
detailsFile=${2/.bam/}-$1-cluster-details.txt

echo -e "making bam of reads that cover $1"
samtools view -bh $2 $1 > $bamFile

echo -e "gathering cluster details"
$java -server -Xms8g -Xmx8g -cp ~/software/Marianas.jar org.mskcc.marianas.umi.duplex.debug.ClusterDetailsAtPosition $bamFile $3 1 2 $referenceFasta $detailsFile
