...
zcat $1 | awk '{if(NR%4!=1) next; split($0, a, ":"); barcode=a[length(a)]; barcodes[barcode]++}END{for(barcode in barcodes) print barcode"\t"barcodes[barcode]}' | sort -k 2,2nr > $2-barcodes.txt
