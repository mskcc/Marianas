...
do
  ...
  #awk -v sample=$sampleName 'BEGIN{FS=OFS="\t"; base[5]="A"; base[6]="C"; base[7]="G"; base[8]="T"}{max=-1; genotype=-1; total=$5+$6+$7+$8; for(i=5; i<=8; i++){if($i>max){max=$i; genotype=i}} for(i=5; i<=8; i++){if(i==genotype) continue; if($i>=total*0.02) next} genotypeCount+=max; altCount+=(total-max);}END{print sample, genotypeCount, altCount, 100*altCount/(genotypeCount+altCount), "Unique" }' $g >> noise.txt

  #awk -v sample=$sampleName 'BEGIN{FS=OFS="\t"; base[5]="A"; base[6]="C"; base[7]="G"; base[8]="T"}{max=-1; genotype=-1; total=$5+$6+$7+$8; for(i=5; i<=8; i++){if($i>max){max=$i; genotype=i}} for(i=5; i<=8; i++){if(i==genotype) continue; if($i>=total*0.02) next}   for(i=5; i<=8; i++){if(i==genotype) continue; substitution=base[genotype]">"base[i]; genotypeCount[substitution]+=max; altCount[substitution]+=$i;}}END{for(s in genotypeCount){g=genotypeCount[s]; a=altCount[s]; print sample, s, g, a, 100*a/(g+a), "Unique"}}' $g >> noise-by-substitution.txt
